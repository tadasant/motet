...
from motet_inference.llm.openrouter import OpenRouterClient, build_payload
...
def test_a_resolved_credential_never_reveals_its_secret() -> None:
    ...
    credential = resolve_credential(env={"OPENROUTER_API_KEY": "sk-or-supersecret"})
    assert "supersecret" not in repr(credential)
    assert credential.auth_headers() == {"Authorization": "Bearer sk-or-supersecret"}
...
def test_a_dropped_reasoning_config_is_logged_even_when_tolerated() -> None:
    ...
    response = client.complete(a_request(reasoning=Reasoning(effort="low", require_evidence=False)))
    assert not response.reasoning_applied
