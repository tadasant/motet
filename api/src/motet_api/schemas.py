...
class EpisodeBuildProgress(BaseModel):
    ...
    **Two fields, not one.** ``step`` is which work; ``stage`` is what is happening to it.
    ...
    estimate_ms: int | None = Field(
        description=(
            "A rough total, the median of the caller's own last few finished episodes. "
            "**An estimate, and clients must label it as one.** Null when fewer than "
            "three have finished, because there is then no sound basis for a number — "
            "show the elapsed time and the step instead of inventing one. Null for an "
            "episode that is no longer building."
        )
    )
    estimate_samples: int = Field(
        description=(
            "How many finished episodes were available to compute ``estimate_ms`` from, "
            "so a client can say what it is made of rather than presenting it as a "
            "promise. It is how many were *found*, not how many were used: below the "
            "three-sample floor ``estimate_ms`` is null and this still reports the one "
            "or two there were."
        )
    )
    attempt: int = Field(
