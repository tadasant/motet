"""What the pipeline records about a run it has finished — motet#25.

The defect: the work happened, the evidence was thrown away. It is not testable by
asserting on a return value, because there *is* none — what it produces is a log line and
a counter, so these tests read the log the worker actually wrote.
...
SPOKEN_TEXT = "Acme announced the round on Tuesday."
...
def _script_payload(news_item_id: str, source_item_id: str) -> object:
    claims = [{"text": SPOKEN_TEXT, "quote": QUOTE, "source_item_id": source_item_id}]
...
class TestCostIsAttributable:
    def test_scripting_an_episode_leaves_a_cost_line_carrying_the_episode_id(
        ...
        install_real_adapters(
            monkeypatch,
            script_payload=_script_payload(news_item_id, source_item_id),
        )
        ...
        assert "cost 1 completion(s)" in line
