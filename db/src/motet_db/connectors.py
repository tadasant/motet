"""PROTOTYPE — connectors: the credentials an agentic enrichment step logs in with.

Two kinds. A ``site`` connector is a username and an optional password for one domain,
and it is what tells the pipeline that an article on that domain is worth a browser
session. An ``mcp`` connector is a remote MCP server the user has authorized over OAuth,
handed to the agent as a tool source.

**The same rules as :mod:`motet_db.phase2`'s credential functions, and the same seam.**
Nothing here commits. The secret never crosses this module in the clear except at the
two boundaries: :func:`create_connector` and :func:`store_connector_secret` take a
:class:`~motet_vault.DekWrapper` and seal; :func:`load_connector_secret` takes a
:class:`~motet_vault.KeyManager` and opens — so the API, which holds the first and not
the second, can write a credential and cannot read one back (invariant 8).

**The AAD is ``user_id:connector_id:kind``.** It is built through
:func:`motet_vault.aad`, whose middle slot is named ``source_id`` because that is what it
was written for; the *string* is the same shape and the property is the same: a
ciphertext moved onto another connector row, or onto a row of the other kind, fails to
authenticate rather than logging one credential into another's site.
"""
...
#: What `GET /v1/connectors` and every mutation return — everything on the row except the
#: envelope. No function in this module returns the sealed bytes to a caller.
...
def normalize_domain(raw: str) -> str:
    ...
    Lowercase, scheme and path and port stripped, a leading ``www.`` dropped. The agent
    matches an article's host against this, so two spellings of one site must not become
    two rows — the unique index on ``(user_id, domain)`` is what enforces it, and this is
    what makes the index mean "one site" rather than "one string".
    """
...
def create_connector(
    ...
    *,
    ...
    A ``site`` row starts ``ready`` even with no password: a passwordless login is a
    complete credential for a site that authenticates by emailed code. An ``mcp`` row
    starts ``needs_auth`` — nothing about it works until the OAuth flow has produced a
    token set, and the row exists first so the flow has something to bind its state to.
    """
    ...
    row = _one(
        ...
        INSERT INTO connectors
            (id, user_id, kind, label, domain, domains, url, username,
             ciphertext, nonce, wrapped_dek, backend, key_name, status)
...
def list_connectors(conn: psycopg.Connection[Any], user_id: str) -> list[StoredConnector]:
    rows = _all(
        ...
        WHERE user_id = %s ORDER BY kind, created_at, id
        """,
        (user_id,),
    )
...
def store_connector_secret(
    ...
    *,
    ...
    The AAD is rebuilt from the row's own ``user_id`` and ``kind``, never from the
    caller, for the same reason :func:`load_connector_secret` does it: the binding is
    only worth having if the row is the authority on what it is bound to.
    """
...
def load_connector_secret(
    ...
    ``None`` when the connector exists without a secret (a passwordless site login) as
    well as when it does not exist; the caller that cares about the difference has the
    row already.
    """
    row = _maybe_one(
        conn,
        """
        SELECT user_id, kind, ciphertext, nonce, wrapped_dek
